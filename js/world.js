class World {
    constructor(
        ...
        this.buildingMiLength = buildingMiLength;
    ...
    #generateBuildings() {
        ...
        for (const seg of this.graph.segments) {
            tmpEnvelopes.push(
                new Envelope(
                    seg, this.roadWidth + this.buildingWidth + this.spacing * 2,
                    this.roadRoundness
                )
            )
        }
        const guides = Polygon.union(tmpEnvelopes.map((p) => p.poly))

        for (let i = 0; i < guides.length; i++) {
            const seg = guides[i];

            if (seg.length() < this.buildingMiLength) {
                guides.splice(i, 1);
                i--;
            }
        }
        ...
        for (const seg of guides) {
            ...
            const buildingCount = Math.floor(
                len / (this.buildingMiLength + this.spacing)
            )
